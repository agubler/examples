		...
		return (
			<li id='todo-item' classes={this.classes(css.todoItem, Boolean(todo.editing) ? css.editing : null, Boolean(todo.completed && !todo.editing) ? css.completed : null)}>
				<div classes={this.classes(css.view)}>
					<input id='toggle' classes={this.classes(css.toggle)} type='checkbox' checked={todo.completed} onchange={this.toggleTodo} />
					<label classes={this.classes(css.todoLabel)} innerHTML={todo.label} ondblclick={this.editTodo} />
					<button id='destroy' onclick={this.removeTodo} classes={this.classes(css.destroy)}/>
				</div>
				{todo.editing ? (
					<input afterCreate={this.afterCreate} onkeyup={this.updateTodo} onblur={this.updateTodo} value={todo.label} classes={this.classes(css.edit)} />
				) : (null)}
			</li>
		);
	}
